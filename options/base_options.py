...
def get_gpu_model():
    ...
    for line in lines:
        parts = line.split(':')
        if len(parts) >= 2:
            # Extract GPU index and details
            index = parts[0].strip().split(' ')[1]
            details = parts[1].strip()
            gpus.append({'index': index, 'details': details})

    # Display the parsed information
    for gpu in gpus:
        print(f"GPU Index: {gpu['index']}, Details: {gpu['details']}")
    return gpus
...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--load_init_multitask_models', type=str, default=None,
                            help='the path to load the saved multitask models from, if None will use the general path')
        ...
        parser.add_argument('--netD', type=str, default='basic', choices=['basic', 'n_layers', 'pixel', 'patch'],
        ...
        parser.add_argument('--netG', type=str, default='resnet_4blocks',
                            choices=['global', 'NormalNet', 'local', 'encoder', 'resnet_7blocks', 'resnet_9blocks',
                                     ...
                                     'unet_small', 'resnet_cat', 'drit', 'drit_orig', 'noise_aware'],
        ...
        parser.add_argument('--tensorboard_path', default=None, type=str,
                            help='Address for saving the tensorboard files')
        ...
        # load_second_sample is a flag to load the second sample in the dataset.
        parser.add_argument('--load_second_sample', action='store_true',
                            help='if specified, load the second sample in the dataset')
    ...
    def gather_options(self):
        ...
        if opt.config_file is not None:
            config = load_yaml_config(opt.config_file)
            # Update argparse values with those from the YAML file
    ...
    def parse(self):
        ...
        if opt.config_file is not None:
            ...
            if isinstance(opt.inshape, str):
                opt.inshape = [int(x) for x in opt.inshape.split()]
        ...
        # gpus = get_gpu_model()  # todo: this is a hack, slurm should handle this
        # # find 3090 in the gpus, if name includes 3090, use it
        # for gpu in gpus:
        #     if '3090' in gpu['details']:
        #         opt.gpu_ids = [int(gpu['index'])]
        #         break
